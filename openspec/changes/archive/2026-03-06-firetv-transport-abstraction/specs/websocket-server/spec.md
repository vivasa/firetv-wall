...
All WebSocket messages SHALL be JSON text frames with either `cmd` (phone→TV) or `evt` (TV→phone) as the top-level discriminator key. The supported commands are: `ping`, `pair_request`, `pair_confirm`, `auth`, `sync_config`, `play`, `stop`, `pause`, `resume`, `seek`, `skip`, `get_state`. The WebSocket server SHALL implement `ClientTransport` and forward raw message strings to its `ClientTransport.Listener` for parsing and routing, rather than parsing commands internally.

#### Scenario: Valid message received
- **WHEN** the server receives a text frame containing valid JSON with a `cmd` field
- **THEN** the raw JSON string is forwarded to `ClientTransport.Listener.onMessageReceived`
- **AND** the listener routes it to the appropriate command handler

#### Scenario: Invalid message received
- **WHEN** the server receives a text frame that is not valid JSON or lacks a `cmd` field
- **THEN** the server forwards the raw string to the listener
- **AND** the listener responds with `{evt: "error", message: "invalid message format"}`
