...
**Reason**: Connection status moves from a dedicated card on the TV tab to a compact chip in the Player Home top bar. The chip shows a colored dot and device name.
**Migration**: Use the device selector chip on Player Home's top bar for connection status. The green/grey dot provides equivalent visual feedback.

### Requirement: TV discovery and pairing
**Reason**: Discovery and pairing move out of the playback remote view into the onboarding flow and Settings → Devices section.
**Migration**: Device discovery is handled by the onboarding-pairing flow (first launch) and Settings → Devices (subsequent). The expanded now-playing view includes a device selector chip for switching between paired devices.
