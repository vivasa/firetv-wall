...
The system SHALL track which preset is currently active. Only one preset SHALL be active at a time. The active preset determines the YouTube URL used for playback. The phone's config store is the authority; the TV caches the active preset index.
...
- **AND** if connected, a config bundle push triggers playback of the new preset
